...
class indexCalculation:
    ...
    def __init__(self,ROI1,Map):
        self.ROI1= ROI1
        self.Map = Map
    def main(self):
        if not self.ROI1:
              self. ROI1=ee.FeatureCollection('users/muthamijohn/kwanza_constituency')
        else:
        #TODO fix logic  
                #cloud mask function
                ...
                S2=ee.ImageCollection('COPERNICUS/S2').filter(ee.Filter.calendarRange(2017,2021, 'year')).filter(ee.Filter.calendarRange(4, 7, 'month')).map(maskS2clouds).filter(ee.Filter.lt('CLOUDY_PIXEL_PERCENTAGE', 5)).filterBounds(self.ROI1)
                # Define a function to compute the indices for a single image
                ...
                def getDataFrame():
                        ...
                        image = collection_with_indices.filterBounds(self.ROI1).median()
